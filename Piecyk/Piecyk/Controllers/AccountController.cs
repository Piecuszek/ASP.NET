...
namespace Piecyk.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            var User = context.Users.Where(u => u.UserName == model.Email).ToList().FirstOrDefault();
            if (User == null)
            {
                return View();
            }

            if (context.UserInfoModels.Where(u => u.ID == User.Id).Count() > 0)
            {
                var UserData = context.UserInfoModels.Where(u => u.ID == User.Id).FirstOrDefault();
                User.UserInfoData = UserData;
            }

            //!
            //var UserData = context.UserInfoModels.Where(u => u.ID == User.Id).ToList().First();
            //User.UserInfoData = UserData;
